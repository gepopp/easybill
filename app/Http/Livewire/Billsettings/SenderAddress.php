...
class SenderAddress extends Component
{
    ...
    protected $rules = [
        ...
        'settings.customer_number' => 'required|integer|min:1',
    ...
    public function mount()
    {
        ...
        foreach ($this->settings as $key => $setting) {
            ...
            $saved = DB::table('bill_settings')
                ->where('user_id', \Auth::id())
                ->where('setting_name', $key)->value('setting_value');

            $this->settings[$key] = !empty($saved) ? $saved : $this->settings[$key];
        }
    }
